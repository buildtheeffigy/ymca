...
const Home = () => {
  ...
  //These update cookie info based on user input
  ...
  //Updates the variables that will be submutted for program creation
  ...
  //generates the User_id cookie, and the new_family_name cookie
  ...
  //Gets the list of family members from the database when page initially loads
  useEffect(() => {
  ...
  const handleQuery = () =>{
    ...
    const results = programss.filter(post => {//Filter by name
    ...
    .filter(post =>{//Only show ones created by this user
      return post.teacher_id==JSON.parse(Cookies.get('user_id')).id;
    })
    .filter(post => {//filters by weekday
  ...
  //Not a "useEffect" because it updates the program after the user changes something, such as creating a program.
  ...
  //Initial programs loader
  useEffect(() => {
      fetchAllSchedules()
  }, [])
  //Cancels the program with the provided ID.
  ...
  //Used for "collapsible" divs. Is the function that actually collapses them
  ...
  //Used to make sure that when the staff creates any new programs, it doesn't conflict with their schedule.
  //dayssString=The "encoded" string for the new classes weekly schedule
  const filtLoad = (dayssString) =>{
    function dsf(value){
      let dayss=dayssString.length-1;
      //Checks if dates would even allow for conflicts
      ...
      (value.start_date==document.getElementById("start").value && value.end_date==document.getElementById("end").value)){
          while(dayss>=0){
              ...
              && dayssString.charAt(dayss)!='0' && value.day_of_week.includes(dayssString.charAt(dayss))){//Old class starts in the middle of new class
                return value;
              }
              else{
                if(value.start_time<=document.getElementById("start_time").value && value.end_time>=document.getElementById("start_time").value
                && dayssString.charAt(dayss)!='0' && value.day_of_week.includes(dayssString.charAt(dayss))){//Old class ends in the middle of new class
                  return value;
                }
                else{
    ...
    if (results[0]!=null){//Searches for enrolled conflicts
      console.log(results[0])
      return true;
    }
    const results2=programss.filter(dsf);
    if (results2[0]!=null){//searches for created programs conflicts
  ...
  const handleClickNex = async e =>{
    ...
    try{
        ...
        document.getElementById("success").innerHTML="";//Resets success message
        var q = new Date();//Used for checking if new programs date is earlier than current date
        ...
        //Checking is at least one weekday has been selected
        ...
        else{//If a weekday has been selected, encode the weekly schedule
        ...
        if(filtLoad(day_time.toString())){//Check that the encoded schedule doesn't cause conflicts. If it does, throw error
          document.getElementById("redtextfinal").innerHTML="You are already hosting/enrolled in a program in this time frame!";
        }
        var numInvalids=0;//if any errors have been thrown, this is where they will be acknowledge and the program stopped
        for(let i=0; i<assd.length; i++){
          if(assd[i].innerHTML!=""){
            numInvalids+=1
          }
        }
        if(numInvalids==0){//No errors, then, continue with program creation
          ...
          if(document.getElementById("prerequisite").checked){//If program has prereq
            ...
            if(prereq_id.data.length != 0){
                  ...
                  const res2=await axios.post("http://localhost:8802/staffschedule",{teach_id:(JSON.parse(Cookies.get('user_id')).id)});//Updates front end schedules
                  const res = await axios.post("http://localhost:8802/personalschedule", {user_id: JSON.parse(Cookies.get('user_id')).id});
                  state.list = res.data
                  state.list2 = res2.data.filter(post =>{
                  return post.teacher_id==JSON.parse(Cookies.get('user_id')).id;
            ...
            <div>
              <div id="notifications" style={{display:"none", width:"100vw", marginLeft:"0vw", marginRight:"0vw", background:"lightyellow", paddingTop:"1px"}}>{/*notifications divs*/}
                ...
                {state.list.map(schedule=>(
                  schedule.canceled==1 ? <div>{schedule.name}, {schedule.day_of_week}, {schedule.start_time} - {schedule.end_time}, {schedule.start_date.toString().split('T')[0]} - {schedule.end_date.toString().split('T')[0]}<span style={{display:"none"}}>{document.getElementById("notifications").style.display="block"}</span></div>
                  :<span></span>
                ))}
              </div>
                ...
                <div>
                  <table class='table'>
                      ...
                      <tbody>
                        <tr>
                          <td>
                            <label for="searchname" style={{fontSize:"20px"}}>Search by</label><input type="search" id='searchname' name='name' placeholder='Name' value={query} onChange={handleQuery}/>
                          </td>
                          <td>
                        ...
                        {state.list2.map( programss=>(
                            <tr key={programss.id}>
                                ...
                                <td>{programss.day_of_week.includes('1') ? <span>M </span>:<span></span>}
                                    ...
                                    {programss.day_of_week.includes('7') ? <span>Su </span>:<span></span>}
                                </td>
                  ...
                  <table class='table'>
                    ...
                    {state.list.map(schedule=>(
                      <tr key={schedule.id}>
                        ...
                        {schedule.canceled==1 ? <td style={{textDecoration:"red line-through"}}>
                        ...
                        {(document.cookie && (JSON.parse(Cookies.get('user_id')).private == 1 || JSON.parse(Cookies.get('user_id')).membership_status == 2)) ? (<td>${schedule.member_price}</td>)
