...
const Help = () => {
  return (
    ...
    {!document.cookie ? <div><div><h1>How To Create An Account</h1></div> <div>To create an account, go to the home page, and fill in
  ...
  create an account with the supplied info. NOTE: If an account with the provided email already exists, you won't be able to create a new one.</div><div><h1>How To Log In</h1></div><div>
...
button below that</div></div>:<span></span>}
<div>
  <div>
    <h1>
    Program Search
...
{document.cookie && JSON.parse(Cookies.get('user_id')).private==2 ? <div>
<div><h1>Other Searches</h1></div>
<div>Admins can not enroll in programs, but can look at and search the programs, users, and registrations databases. The search features are mostly the same, except some paramaters have two check boxes: you must check the top box
to begin filtering, and the second box toggles which values are filtered out.</div>
<div><h1>Hard Delete</h1></div>
<div>Admins have the ability to completely delete a user's account after said account has been soft deleted. To do this, go to "Users", find the account to delete, and then click the HARD DELETE button next to their info.</div>
</div>:<span></span>}

  {document.cookie && JSON.parse(Cookies.get('user_id')).private!=2 ? <div>
    <div><h1>Enrolling In Programs</h1></div>
    <div> To enroll in a program, go to the Programs page, find the program you'd like to enroll in, and click the "Enroll" button. You will be taken to another page. If you are Currently
    ...
    that you can't enroll in programs that are currently at max capacity, or canceled.</div>
    <div><h1>Canceled Programs</h1></div>
    <div>You can't enroll in canceled programs. These programs will have a line going through them. If a program you're enrolled in gets canceled, you'll get a notice on your user dashboard, and it will be striked through
    on your schedule.</div>
    </div>:
    <span></span>}

    {document.cookie && JSON.parse(Cookies.get('user_id')).private==1 ? <div>For staff, enrollment is basically the same, except if the conflict occures with a program that the staff member is hosting, there will be no option
    to drop the conflicts. They'll have to be canceled from the home page.</div>:<span></span>}

{document.cookie && JSON.parse(Cookies.get('user_id')).private==0 ? <div>
  <div><h1>User Dashboard</h1></div>
<div>After you've logged in, you'll be brought to the user dashboard. From here, you can check the information onf what programs you're enrolled in or drop out of them, change your membership status, or convert your account
 to a family account and add members.</div>
<div><h1>Family accounts</h1></div>
<div>Family members all share one memberment and password, while being able to enroll in their own choice of programs. To create a family account, click on the "Family" tab of the user dashboard, and then click "create family".
...
each family member to see that member's schedule of programs.</div>
<div><h1>Memberships</h1></div>
<div>YMCA Members get a discount on programs. That's it. You can view or change your membership status by going to your user dashboard, and clicking on the "Membership" tab.</div>
<div><h1>Schedule and Dropping Programs</h1></div>
<div>To view what programs you're enrolled in, when they're held, and their cost, go to your user dashboard and open the "Schedule" tab. You can also drop out of programs from here by clicking the "Drop class!" button next
to the program's info.</div></div>:<span></span>}

{document.cookie && JSON.parse(Cookies.get('user_id')).private==1 ? <div>
<div><h1>User Dashboard</h1></div>
<div>From the user dashboard, you can create new programs, check and cancel your own programs, and view or drop out of programs you're enrolled in</div>
<div><h1>Creating Programs</h1></div>
<div>To create a new program, go to your user dashboard, and click the "Create NEW class" tab. This will give you a few text fields.
...
creating would conflict with any programs you are currently enrolled in or hosting, you won't be able to create the program.</div>
<div><h1>Your Programs and Canceling</h1></div>
<div>By clicking on the "Created classes" tab of the user dashboard, you can see the scheule of programs you're hosting, how many people are in them, and their costs. You can
also cancel your programs from here, by clicking on the "cancel" button to the right of each program. Canceled programs cannot be recovered.</div>
<div><h1>Schedule and Dropping Programs</h1></div>
<div>To view what programs you're enrolled in, when they're held, and their cost, go to your user dashboard and open the "Schedule" tab. You can also drop out of programs from here by clicking the "Drop class!" button next
to the program's info.</div></div>:<span></span>}
